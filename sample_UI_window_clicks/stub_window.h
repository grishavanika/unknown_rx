...
public:
    ...
    using Handlers = std::unordered_map<UINT, MessageCallback>;
    ...
    explicit StubWindow(const char* class_name, Handlers startup_handlers = Handlers())
        : class_name_(class_name)
        , wnd_handler_()
        , handlers_(std::move(startup_handlers))
        , wnd_(create())
    {
    }
